...
async def get_all(db: Session = Depends(database.get_db), Token:str= Header()): 
    email = await verify_token(Token)
    return blog.get_all_blogs(email, db)

@router.delete('/{id}', status_code=status.HTTP_204_NO_CONTENT)
def destroy(id: int, db: Session = Depends(database.get_db)):
    return blog.destroy(id, db)

@router.get('/output', response_model=List[schemas.ShowBlogAnalysis])
...
@router.get('/output_for_date_range', response_model=List[schemas.ShowBlogAnalysis])
...
async def classify_blog_id(blog_id:int, db: Session = Depends(database.get_db),Token:str= Header()):
    email= await verify_token(Token)
    return await blog.classify_blog_id(email, blog_id, db)

@router.get('/suggest')
def get_content(blog_id:int, Token:str= Header(), db: Session = Depends(database.get_db)):
    return suggestions.get_content(blog_id, db)

@router.get('/{email}',status_code=200, response_model=schemas.ShowBlogAnalysis)
