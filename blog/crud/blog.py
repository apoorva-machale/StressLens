...
from blog.crud.permissions_subscriptions import is_premium
...
def get_all_blogs(email, db: Session):
    try:
...
async def analyze_blog(request: schemas.BlogBase, email, db: Session):
    try:
        ...
        if user:
            ...
            if word_count < 25:
                raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Insufficient word count to be considered as a blog")
            else:
                if is_premium(email, db):
                    ...
                    for category in classifier_output:
                        ...
                        db.commit()
                        db.refresh(category)
                    return "Saved blog with sentiment and classification analysis"
                else:
                    _id = user.id
                    sentiment_blog = models.Blog(title=request.title, body= request.body, user_id=_id, creation_time=datetime.now())
                    db.add(sentiment_blog)
                    db.commit()
                    db.refresh(sentiment_blog)
                    return "Blog details saved successfully"
    except Exception as e:
        print(f"Error classifying blog: {e}")
        return None

async def classify_blog(date, email, db: Session):
    if not is_premium(email, db):
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Not a premium member")
    try:
        user = db.query(models.User).filter(
            models.User.email == email
        ).first()
...
async def classify_blog_id(email, blog_id, db: Session):
    if not is_premium(email, db):
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Not a premium member")
    try:
        classification = db.query(models.Category).filter(
        ...
        if not classification:
            print("No classification present for the given blog_id")
        else:
            return classification
    except Exception as e:
        print(f"Error classifying blog: {e}")
