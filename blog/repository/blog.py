...
def classify_blog(request: schemas.Blog, db: Session):
    output = classify_text(request.body)
    # new_blog = models.Blog(title=request.title, body= request.body,user_id=1, creation_time=datetime.now(),)
    # db.add(new_blog)
    # db.commit()
    # db.refresh(new_blog)
    return output
